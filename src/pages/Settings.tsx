...
import { Card, Tabs, Form, InputNumber, Input, Button, Select, Table, Space, message, Popconfirm, Modal, Row, Col, Switch, Tag, Alert, Descriptions, Skeleton, Empty, Typography, Upload } from 'antd';
...
import type { ProductTemplate, Settings, OperationLog, AuditModule, Customer, Transaction, AfterSales } from '@/types';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    Promise.all([loadTemplates(), loadSettings(), hasData().then(setHasExistingData)]).finally(() => setLoading(false));
  }, []);
  ...
  const tplColumns = [
    ...
    { title: '质保天数', dataIndex: 'warranty_days', width: 90, align: 'center' as const },
    {
      title: '启用',
      dataIndex: 'is_active',
      width: 70,
      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            children: (
              <>
                <Button type="primary" icon={<PlusOutlined />} style={{ marginBottom: 16 }} onClick={() => { setEditingTpl(null); tplForm.resetFields(); setTplModalOpen(true); }}>
                  新增模板
                </Button>
                <Table rowKey="id" dataSource={templates} columns={tplColumns} size="middle" pagination={{ pageSize: 20 }} locale={{ emptyText }} />
              </>
            ),
          },
          {
            ...
            children: (
              <Form form={settingsForm} layout="vertical" style={{ maxWidth: 600 }}>
                <Form.Item name="warranty_days" label="默认质保周期（天）" rules={[{ required: true }]}>
      ...
      >
        <Form form={tplForm} layout="vertical">
          ...
          <Row gutter={12}>
            ...
            <Col span={12}>
              <Form.Item name="warranty_days" label="默认质保天数" rules={[{ required: true }]}>
                <InputNumber min={1} max={3650} style={{ width: '100%' }} />
              </Form.Item>
            </Col>
          </Row>
