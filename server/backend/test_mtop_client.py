...
class TokenRefreshTests(unittest.TestCase):
    def test_refresh_updates_token_and_matching_encryption_cookie(self):
        client = MtopClient(
            "unb=1; _m_h5_tk=oldtoken_1000; _m_h5_tk_enc=oldenc"
        )
        response = httpx.Response(
            200,
            headers=[
                ("set-cookie", "_m_h5_tk=newtoken_2000; Path=/"),
                ("set-cookie", "_m_h5_tk_enc=newenc; Path=/"),
            ],
        )

        refreshed = client._refresh_token_from_response(response)

        self.assertTrue(refreshed)
        self.assertEqual(client.token, "newtoken")
        self.assertEqual(client.cookies["_m_h5_tk"], "newtoken_2000")
        self.assertEqual(client.cookies["_m_h5_tk_enc"], "newenc")
        self.assertIn("_m_h5_tk_enc=newenc", client.cookie_str)
        self.assertTrue(client.cookies_changed)
