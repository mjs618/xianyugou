...
1. GET /api/metrics 读端点 100/min
2. 实际触发：超过读端点限额返回 429
"""
import unittest
...
class MetricsRouteRateLimitDecorationTests(unittest.TestCase):
    """metrics 路由所有端点必须挂载 @limiter.limit。"""

    def test_metrics_read_endpoints_limited_to_100_per_minute(self):
        _assert_rate_limit(self, "app.routers.metrics.get_metrics", "100")


@pytest.mark.rate_limit
class MetricsRateLimit429Tests(unittest.IsolatedAsyncioTestCase):
    """实测：超过 metrics 读端点限额返回 429。"""
    ...
    async def asyncSetUp(self):
        ...
        token_module.get_api_token = lambda: "rate-limit-test-token-metrics"
        try:
    ...
    async def test_get_metrics_returns_429_on_101st_request(self):
        """GET /api/metrics 限流 100/min：第 101 次应返回 429。"""
